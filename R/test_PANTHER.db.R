...
.test <- function(dir, pattern = "^test_.*\\.R$")
{
	...
	if (missing(dir)) {
		dir <- system.file("unitTests", package="PANTHER.db")
		if (!length(dir)) {
			dir <- system.file("UnitTests", package="PANTHER.db")
			if (!length(dir))
	...
	suite <- RUnit::defineTestSuite(name="PANTHER.db RUnit Tests", dirs=dir,
	...
	if (length(details <- .failure_details(result)) >0) {
		...
		cat("\n\n")
		stop("unit tests failed for package PANTHER.db")
	}
	result
}
