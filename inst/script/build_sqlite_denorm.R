...
panther_folder<-"/Downloads/"
schema.text<-paste(read.delim(paste0(home_folder,"PANTHER_DB.sql"),stringsAsFactors=F)[,1],collapse="\n")
### ###
...
dir.class<-file.path(panther_folder,src.class)
dir.class_rel<-file.path(panther_folder,src.class_rel)
...
if(T){
  ...
  filenames <- unlist(strsplit(getURL(src.ortho, dirlistonly = TRUE), "\n"))
  sapply(filenames,function(fn){download.file(file.path("ftp:/",src.ortho,fn),file.path(dir.ortho,fn), cacheOK = TRUE, quiet = FALSE,method="wget",extra=c("-nv","-t 3","-nc","-T 5"))})
...
if(T){
  ...
  save(data_entrez,file="data_entrez.RData")
}
#load("data_entrez.RData")
### ###
...
insert_df(db,"protein_class_child",":class_tree_id,:child_class_id",data_class_rel_child)
### ###
...
insert_df(db,"protein_class_ancestor",":class_tree_id,:ancestor_class_id",data_class_rel_off)
### ###
